...
public class HUD : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        if (PlaneTransform == null || PlaneRigidBody == null)
            Debug.LogError($"<color=red>Ошибка:</color> На объекте {gameObject.name} не назначены ссылки на PlaneTransform или PlaneRigidBody!");
        return;

        Vector3 velocity = (PlaneRigidBody.linearVelocity.sqrMagnitude > 1) ? PlaneRigidBody.linearVelocity : PlaneTransform.forward;
        ...
        // SPEED
        var speed = PlaneRigidBody.linearVelocity.magnitude;
